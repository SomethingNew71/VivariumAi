<script lang="ts" setup>
  ...
  async function submitPrompt() {
    ...
    const currentMsg = userPrompt.value;
    userPrompt.value = '';
    chatList.value.push({
      text: currentMsg,
      isUser: true,
    ...
    await useFetch(() => `/api/chat`, {
      ...
      if (chatData.value) {
        // @ts-ignore
        console.log(chatData.value.state);
        chatList.value.push({
          // @ts-ignore
          text: chatData?.value?.state.turns.pop()?.output.content,
          isUser: false,
        });
      }
    });
  ...
  >
    <client-only>
      <div class="card xl:card-side bg-base-300 shadow-xl">
        ...
        <div
          class="card-body lg:min-w-96 lg:max-w-128 xl:min-w-128 xl:max-w-3xl"
        >
          <div
            class="chat-container bg-base-100 min-h-96 max-h-192 overflow-y-auto pb-32 px-4 pt-3"
          >
            <div
      ...
      <div class="card-actions justify-between pt-3">
        <p><strong>Assistant: </strong>{{ assistant_id }}</p>
        <p><strong>Thread: </strong>{{ thread_id }}</p>
      </div>
      <dialog id="restart_modal" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box">
          ...
          <p class="py-4">
            All of your progress will be lost forever and you will need to
            restart from the beginning. This is a one way street...
          </p>
          <div class="modal-action">
            <form method="dialog">
              <!-- if there is a button in form, it will close the modal -->
              <button class="btn btn-error" @click="restartGame()">
                Close
              </button>
            </form>
          </div>
